...
description: >-
  ...
  walks the compliance checklist for every layer the diff touches, reads the
  pull request's recorded decisions first so justified deviations are not
  reported as findings, reads test bodies instead of counting them, ranks
  findings blocking / should fix / consider, and ends with an explicit
...
Last of the phases, after [`TICKET-ANALYSIS.md`](https://github.com/konradcinkusz/architecture-standards/blob/main/docs/delivery/TICKET-ANALYSIS.md) and
