...
description: >-
  ...
  unsure whether a ticket is ready to implement at all. Reads the ticket
  against the architecture rather than against the existing code, maps every
  acceptance criterion to a layer and at least one file, names the guides
  implementation will need, flags principle-level deviations while they are
  still cheap, runs a read-only exploratory round, separates blocking
  questions from assumptions worth documenting, and tests the three conditions
  that gate entry into implementation.
