...
description: >-
  Use when exercising an API change over HTTP against the application already
  running on your own machine, after the automated tests are green and before
  opening the pull request. Resolves the base URL from the solution itself —
  the Aspire AppHost, the service's launchSettings profile, or the running
  dashboard — rather than from a remembered port; checks /health and /alive
  first and captures which optional integrations are degraded, so a correctly
  degrading endpoint is not filed as a 500; confirms the running process is
  the build under test; then exercises the acceptance criteria plus the
  regression, contract-shape and negative cases, in the estate's manual
...
and [`SERVICE-API-PATTERNS.md`](https://github.com/konradcinkusz/architecture-standards/blob/main/docs/guides/SERVICE-API-PATTERNS.md) — because
...
- **Never reuse these addresses for anything deployed.** P1 says the AppHost is not the
  ...
  runtime. Deployed environments are [`CLOUD-TEST.md`](https://github.com/konradcinkusz/architecture-standards/blob/main/docs/delivery/CLOUD-TEST.md)'s job, and its target
...
[`TESTING-STRATEGY.md`](https://github.com/konradcinkusz/architecture-standards/blob/main/docs/guides/TESTING-STRATEGY.md) §7 so the result reads like every
...
alongside the plan from [`IMPLEMENTATION-PHASE.md`](https://github.com/konradcinkusz/architecture-standards/blob/main/docs/delivery/IMPLEMENTATION-PHASE.md) §4 — and
...
Generated from [`docs/delivery/TEST-ON-LOCALHOST.md`](https://github.com/konradcinkusz/architecture-standards/blob/main/docs/delivery/TEST-ON-LOCALHOST.md) by `scripts/build-marketplace.mjs`. Do not edit this file: change the source document, or its entry in `catalog/marketplace.catalog.json`, and re-run the generator.
