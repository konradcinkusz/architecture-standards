...
// Front-matter keys a procedure skill may carry beyond name and description. Each one
// changes who may invoke the skill or what it may do while active, so the set is closed:
// a typo here would silently disable a gate rather than fail a build.
...
// A procedure skill is invoked, not consulted. `/implementation-phase` has to *be* the
// checklist — in order, in the body — so the document is emitted verbatim rather than
// summarised, and there is no second copy under references/ to drift from it. The
// knowledge shape in buildSkill is the opposite artifact and stays as it is.
function buildProcedureSkill(skill, sourcePath, raw, baseUrl) {
  const front = ['---', `name: ${skill.name}`, yamlFolded('description', skill.description)];
  for (const [key, value] of Object.entries(skill.frontMatter ?? {})) {
    if (!PROCEDURE_KEYS.has(key)) {
      console.error(
        `${skill.name}: unknown front-matter key "${key}".\n` +
          `  Allowed: ${[...PROCEDURE_KEYS].join(', ')}.`
      );
      process.exit(1);
    }
  ...
  const footer =
    `\n\n---\n\nGenerated from [\`${sourcePath}\`](${baseUrl}${sourcePath}) by ` +
    '`scripts/build-marketplace.mjs`. Do not edit this file: change the source document, ' +
    'or its entry in `catalog/marketplace.catalog.json`, and re-run the generator.\n';

  return `${front.join('\n')}${rewriteLinks(raw, sourcePath, baseUrl).trimEnd()}${footer}`;
}
