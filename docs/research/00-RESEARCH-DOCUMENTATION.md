...
- **Install [`HOUSE-PREAMBLE.tex`](HOUSE-PREAMBLE.tex) once per repository**,
  as `docs/research/papers/house-preamble.tex` (or `docs/papers/` for a
  non-study document — see below), and **start the paper from
  [`PAPER-TEMPLATE.tex`](PAPER-TEMPLATE.tex)**, which is a paper-shaped
...
[`CHECK-PAPERS.mjs`](CHECK-PAPERS.mjs) is the mechanical half: copy it to
