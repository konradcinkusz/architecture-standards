...
[`TICKET-ANALYSIS.md`](TICKET-ANALYSIS.md) §7, and it happens because answers stay in
...
guides under [`docs/guides/`](../guides/). **If you cannot, stop and say so.** Feedback
...
|---|---|---|
| what the ticket asks for, or how it maps onto the architecture | the analysis document ([`TICKET-ANALYSIS.md`](TICKET-ANALYSIS.md) §2) | re-test §6's four conditions and report them |
...
| something the generated prompt lost or garbled, where the analysis is right | nothing upstream; it is a generation defect | regenerate ([`GENERATE-MASTER-PROMPT.md`](GENERATE-MASTER-PROMPT.md) §7) |
| the code that came out, where the analysis and prompt are right | nothing upstream | back to [`IMPLEMENTATION-PHASE.md`](IMPLEMENTATION-PHASE.md) |
| a review finding you disagree with | the pull request's recorded decisions | [`PR-REVIEW.md`](PR-REVIEW.md) re-reads them |
...
- **Amend, do not regenerate.** Change the rows the feedback touches and leave the rest
  ...
  the loss behind output that looks the same ([`TICKET-ANALYSIS.md`](TICKET-ANALYSIS.md)
  §2).
...
([`GENERATE-MASTER-PROMPT.md`](GENERATE-MASTER-PROMPT.md) §5).

Where code exists against a row that just changed, **name the files and stop**. Rewriting
them is [`IMPLEMENTATION-PHASE.md`](IMPLEMENTATION-PHASE.md)'s job, with its pre-analysis
