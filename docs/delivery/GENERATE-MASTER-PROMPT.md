...
[`TICKET-ANALYSIS.md`](TICKET-ANALYSIS.md) has been narrowing — and produces **one
...
[`IMPLEMENTATION-PHASE.md`](IMPLEMENTATION-PHASE.md) and is invoked rather than retyped.
...
[`TICKET-ANALYSIS.md`](TICKET-ANALYSIS.md) §2, with its table, its risk list and its
...
- **The document is current.** If the conversation has moved past the file — a correction
  ...
  [`FEEDBACK.md`](FEEDBACK.md). Generating from a stale analysis produces a prompt that
...
[`TICKET-ANALYSIS.md`](TICKET-ANALYSIS.md) §4 applies to reading code.
...
[`TICKET-ANALYSIS.md`](TICKET-ANALYSIS.md) §6 lets a person proceed with a condition
...
([`IMPLEMENTATION-PHASE.md`](IMPLEMENTATION-PHASE.md) §7), not a puzzle to route around.
...
through [`FEEDBACK.md`](FEEDBACK.md), and then this phase runs again.
...
- **The prompt was faithful and the analysis was wrong** — a row named the wrong layer, a
  ...
  [`FEEDBACK.md`](FEEDBACK.md), let it land in the analysis, then regenerate.
