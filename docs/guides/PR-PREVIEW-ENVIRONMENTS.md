...
([`TESTING-STRATEGY.md`](TESTING-STRATEGY.md) §4) and to let a reviewer click the
thing.
...
- Client artifacts built per PR (a browser extension, an installer) are pinned to
  ...
  ([`BROWSER-EXTENSIONS.md`](BROWSER-EXTENSIONS.md) §7), so the preview never serves
...
3. **Legacy aliases**: the destroy list carries historical app-name prefixes from
   before any rename ([`REPO-BASELINE.md`](REPO-BASELINE.md) §4) — teardown that only
